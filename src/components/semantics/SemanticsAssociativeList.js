...
const SemanticsAssociativeList = props => {
  ...
  let elements = associativeList.map((i, index) => {
    ...
    return (
      <div
        ...
        className="keyword-result-borderless"
        key={index}
      >
        {i.item} ({i.weight})
      </div>
  ...
  if (props.isLoading) {
    ...
    component = (
      <div>
        <p>
          Lexemes are displayed in the ascending order sorted by their (relative
          weight), and color-coded by their proximity to the defined concept.
        </p>
        {elements}
        <Divider hidden />
        <div className="tag-powered-by">
          <p>
            Powered by:{' '}
            <a href="Word Associations Network">Word Associations Network</a>
          </p>
        </div>
      </div>
    );
    ...
    component = (
      <p>
        An interconnected list of lexemes that share a common semantic property
        within the defined conceptual domain.
      </p>
    );
  }
