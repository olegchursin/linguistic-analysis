  ...
  if (defObj.results) {
    component = (
      <div>
        <h3>{defObj.word}</h3>
        <p>
          / {defObj.pronunciation.all} / &#9642; Syllables:{' '}
          {defObj.syllables.count} /{' '}
          {defObj.syllables.list.map(syllable => (
            <span key={syllable}>| {syllable} |</span>
          ))}
        </p>
        {defObj.results.map(result => {
          return (
            <Segment key={result.definition}>
              <h4>{result.partOfSpeech}</h4>
              <p>{result.definition}</p>
              {result.examples ? (
                <div>
                  <Divider hidden />
                  <p className="dictionary-wordsapi-subheader">
                    Examples
                    <Popup
              ...
              {result.hasCategories ? (
                <div>
                  ...
                  <p>
                    {result.hasCategories.map(category => (
              ...
              {result.instanceOf ? (
                <div>
                  ...
                  <p>
                    {result.instanceOf.map(instance => (
              ...
              {result.hasInstances ? (
                <div>
                  ...
                  <p>
                    {result.hasInstances.map(instance => (
                      <span key={instance}>{instance}, </span>
                    ))}
                  </p>
                </div>
        ...
        <div className="tag-powered-by">
          <p>
            Powered by: <a href="https://www.wordsapi.com">WordsAPI</a>
          </p>
        </div>
      </div>
    );
    ...
    component = (
      <p>
        Definition provided by <a href="https://www.wordsapi.com">WordsAPI</a>
      </p>
    );
  }
