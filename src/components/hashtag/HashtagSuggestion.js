...
const HashtagSuggestion = (props) => {
  ...
  let elements = suggestionArr.map((hashtag, index) => {
    ...
    if (90 <= hashtag.weight && hashtag.weight <= 100) {
      ...
      return <div style={{backgroundColor: color}} className="keyword-result-borderless-inverted" key={index}>
        <div>{hashtag.item} ({hashtag.pos}): {hashtag.weight}</div>
      </div>
    })
