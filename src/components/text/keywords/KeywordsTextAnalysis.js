...
const KeywordsTextAnalysis = props => {
  ...
  if (props.isLoading) {
    ...
    component = (
      <div>
        {keywords.map(keyword => (
          <div className="keyword-result" key={keyword}>
            {keyword}
          </div>
        ))}
        <div className="tag-powered-by">
          <p>
            Powered by:{' '}
            <a href="http://textanalysisonline.com/">Text Analysis</a>
          </p>
        </div>
      </div>
    );
    ...
    component = (
      <p>
        <a href="http://keywordextraction.net/">Keyword Extraction API</a>{' '}
        provides professional keyword extractor service which is based on
        advanced Natural Language Processing and Machine Learning technologies.
      </p>
    );
  }
