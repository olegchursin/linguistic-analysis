...
class HashtagApi {
  ...
  static hashtagDefinition(inputText) {
    return fetch(
      `https://cors-anywhere.herokuapp.com/http://api.tagdef.com/${inputText}.json`
    ).then(res => res.json());
  ...
  static hashtagSuggestion(suggestInput) {
    return fetch(
      `https://wordassociations-word-associations-v1.p.mashape.com/json/search?apikey=${WORDASSOCIATIONS_KEY}&indent=yes&lang=en&limit=50&pos=noun%2Cadjective%2Cverb%2Cadverb&text=${suggestInput}&type=stimulus`,
      {
        headers: {
